...
module.exports = function(pagerun){
    ...
    function openCallback(errorMessage){
        ...
        if(errorMessage !== null){
            if(errorMessage === 'webdriverError' && openCount < config.retry ){
                self.log('retry', openCount);
    ...
    function openUrl(endCallback){
        ...
        wd.run(function(browser, $){
            ...
            var arrPageResults = browser.exec('var callback = arguments[arguments.length-1];if(window.pagerun){pagerun.checkEnd(function(){callback(pagerun.getResult(true));});}else{callback("[]");}', true);
            if(browser.isOk(arrPageResults)){
                try{
                    arrPageResults = JSON.parse(arrPageResults);
                    arrPageResults.forEach(function(result){
                        pagerun.result('browser', result);
                    });
                }
                catch(e){}
            }
            else{
                self.log('getPageResultFailed', arrPageResults.value.message);
            }
            if(screenshot === true){
